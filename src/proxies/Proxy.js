...
class BaseProxy {
  ...
  setParameters(parameters) {
    Object.keys(parameters).forEach((key) => {
      if (key === 'market' || key === 'marketType' || key === 'currency') {
        this.parameters[key] = parameters[key].toUpperCase();
      } else {
   ...
   * @param {Object|null} dataOrConfig The data to be send with the request or Axios extra config.
   * @param {Object|null} config       Axios extra config.
   *
   * @returns {Promise} The result in a promise.
   */
  submit(requestType, url, dataOrConfig = null, config = null) {
    return new Promise((resolve, reject) => {
      Vue.$http[requestType](url + this.getParameterString(), dataOrConfig, config)
        .then((response) => {
